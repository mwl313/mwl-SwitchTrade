...
class HostTransport:
    ...
    # A안 hybrid: how long the ap-engine path waits for the Switch to associate after
    # the room is open. start() has ALREADY returned by then (ready is set at beacon
    # time), so this budget is teardown-only - human-paced joining must not be rushed.
    AP_ENGINE_JOIN_TIMEOUT = 300.0
    ...
    def _resolve_ap_engine(self):
        """A안 hybrid (docs/19): return a ready-to-start ApEngine when the HOST path should
        use the hostapd backend, else None (stock ldn create_network fallback).

        Policy: engine when use_ap_engine is on AND hostapd is importable/available. The
        engine owns the AP iface directly (ifname, ch6) - the ldn fallback keeps its own
        vif scheme, so the two paths never share state. Any failure here (missing module,
        missing hostapd binary) degrades to None with one log line - never raises, because
        the caller treats None as 'use the fallback'."""
        if not self.use_ap_engine:
            return None
        try:
            from frlgsim.ap_hostapd import HostapdApEngine, EngineNotAvailable  # noqa: F401
        except ImportError as e:                     # engine module missing
        ...
        try:
            ...
            engine = HostapdApEngine(
                ...
                wpa_passphrase=self.ap_passphrase,   # None = open (LDN-style auth)
                log=self.log)
        except Exception as e:                       # noqa: BLE001 - degrade, never raise
            self.log(f"[ap-engine] init failed ({e}) - using ldn fallback")
            return None
        self.log(f"[ap-engine] hostapd backend resolved (iface={self.ifname} "
                 f"ch={self.channel})")
        return engine

    def _init_common_state(self):
        """Attribute initialization shared by both HOST paths (engine + ldn fallback).
        Extracted so __init__ stays linear - called once from __init__ after the path
    ...
    def _run_ldn(self):
        ...
        async def main():
            ...
            keys = ldn.load_keys(self.keys_path)
            # STEP 10 fix (docs/17 D3): the real Switch sets a nonzero u16 at RFU record
            # partner_info[4:6] (measured 0x1584 from DESTROY). Our all-zero room was
            ...
            self.app_data = self._application_data_override or beacon.build_application_data(
            ...
            # -- A안 hybrid (docs/19): ApEngine first, ldn create_network fallback ----
            # The engine path owns beaconing + join detection (hostapd provably
            # beacons on rtl8xxxu where ldn's START_AP does not); the stock ldn path
            # below stays VERBATIM as the fallback (guest-only cards / engine
            # disabled / unknown cards). The beacon-head + START_AP-attrs overrides
            # patch LDN'S OWN AP - so they must only ever run on the fallback path.
            engine = self._resolve_ap_engine()
            if engine is not None:
                async with engine:
                    await engine.start()
                    # Same ready-contract as the fallback: start() returns once the
                    # room is being advertised - joining is human-paced and detected
                    # below (AP-STA-CONNECTED), never blocking the caller's start().
                    self.ssid = str(engine.ssid).encode()
                    self.our_ip = "169.254.21.1"     # participant-0 slot (LDN scheme)
                    self.our_mac = self._iface_mac(engine.iface) or b"\x00" * 6
                    # LiveTransport-compat prefill (mirrors the fallback): host_* start
                    # as ourselves, then the join below retargets them at the Switch.
                    ...
                    self.log(f"[live] hosting ssid={self.ssid.hex()} "
                             ...
                             f"ch={self.channel} (ap-engine)")
                    self.info("Room is open (ap-engine) - waiting for the Switch to scan and join...")
                    self._ready.set()
                    sta_mac = await engine.wait_station(
                        timeout_s=self.AP_ENGINE_JOIN_TIMEOUT)
                    self.info(f"[ap-engine] joined by {sta_mac}")
                    sta_bytes = bytes.fromhex(
                        sta_mac.replace(":", "").replace("-", ""))
                    if len(sta_bytes) == 6:
                        self.host_mac = sta_bytes
                        self.host_ip = "169.254.21.2"   # first joiner slot [APNetwork]
                        self._peer = (self.host_mac, self.host_ip)   # sticky, like _track_peer
                        self.log(f"[live] PEER JOINED our room (the Switch): "
                                 f"{self.host_ip}/{sta_mac} (ap-engine)")
                    # The existing TAP game loop, unchanged: the data plane and the
                    # sim's send/recv contract are identical on both paths.
                    while not self._stop.is_set():
                        await trio.sleep(self.PEER_POLL_INTERVAL)
                        self._track_peer()
                return                    # engine path done - skip the ldn fallback
