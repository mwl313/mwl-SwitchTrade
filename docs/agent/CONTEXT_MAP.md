...
| --- | --- | --- |
| Core simplification | `docs/core-simplification/00_SWITCHTRADE_CORE_MASTER_PLAN.md` and the matching `A_*`, `B_*`, or `C_*` design/prompt pair | Only if a trigger applies |
| Legacy full-product code, tests, or documentation | Relevant `80-abc-connection-architecture-20260829.md` and `FUTURE_TODO.md` sections | Only if a trigger applies |
...
When an incident lookup is triggered, use `docs/incidents/INDEX.md` to locate matching current or
historical entries. Open only the necessary current entry or archive range; do not read the archive
