...
- rtl8xxxu(인커널)는 8188EU만 LDN join이 검증됨. **8192EU는 assoc까지는 되지만 암호화 단계에서 실패** (2026-08-21 실측)
- 칩셋마다 리눅스 드라이버 구현 상태가 다르므로, **칩셋 판별 → 전용 드라이버**가 정답
...
| `0bda:818b` | RTL8192EU | `rtl8192eu-linux` | out-of-tree (Mange, 1.4k★) | 🔄 빌드/검증 중 |
