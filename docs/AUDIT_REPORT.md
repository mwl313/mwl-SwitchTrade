...
| STB-002 / RADIO-002 | A transient USB bus ID was used as device identity, and post-install selection had no elevated share/bind path. Replug/reboot could select the wrong adapter; an unshared card entered an attach/Repair loop. | APIs and persisted selection carry Windows InstanceId plus current bus ID. The native provisioner re-resolves InstanceId for one-time non-forced authorization, verifies sharing, and named gates expose detected/shared/attached/USB-visible/driver/PHY/RX state. Re-enumeration, unshared, identity-change, final-verification, and duplicate-device simulations pass. | **Software fix implemented; physical validation pending** |
...
| WIN-015 | A process death after `wsl --import` but before writing `/etc/switchtrade-distro.json` left the exact newly imported distro markerless. Repair rejected it as foreign and moved a legacy Windows tree before completing the ownership gate. The packaged legacy rootfs also lacked the generic marker that the current builder intended to create. | Recovery distinguishes a missing marker from an unreadable or malformed marker and bootstraps ownership only for a schema-3 fresh-install transaction still at `importing_distro` with the exact recorded Lxss BasePath. Foreign paths and malformed markers remain rejected. All orphan moves now occur after the complete recovery plan validates ownership, and packaging rejects rootfs archives without the generic installer marker. | Fixed; live interrupted-install retry pending |
