"""Bounded, read-only LLSF diagnostics shared by RFU endpoint boundaries.
...
class RfuProgress:
    ...
    def observe(self, slot: bytes, *, parent: bool, timestamp=None, reliable_seq=None):
        ...
        if metadata is not None:
            ...
            if reliable_seq is not None:
                entry["reliable_seq"] = reliable_seq
            ...
            # the three-bit tag. This observes the caller's boundary, not the
            # game's consumption/error state. Eight lost commands can alias it.
