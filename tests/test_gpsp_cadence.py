...
class LegacyReceiptCadence(RfuCadence):
    """Frozen f427535 receipt policy for failure/counterfactual tests ONLY.

    NI recognition still uses production code. Never select this in a runtime.
    """
    ...
    def admit(self, actions):
        ...
        for action in actions:
            ...
            if action.classification == "parent_ack":
                self.receipts_coalesced += self._receipt is not None
                self._receipt = action
                output.extend(self.poll(force=self._ordered_receipts))
            else:
                admitted = super().admit((action,))
                if self._ordered_receipts:
                    output.extend(self.poll(force=True))
                output.extend(admitted)
        return tuple(output)
    ...
    def poll(self, *, force=False):
        ...
        self._receipt_due = self.clock() + .25
...
def test_every_receipt_preserves_original_bytes_number_order_and_retirement():
    clock = [0.0]
    flow = RfuCadence(lambda: clock[0])
    # Timestamp and receipt number wrap, repeats and sparse values stay opaque.
    actions = tuple(receipt(t) for t in (0xFFFFFFFE, 0xFFFFFFFF, *range(1, 1000), 999))
    assert flow.admit(actions) == actions
    assert flow.receipts_coalesced == 0
    assert flow.snapshot()["receipt_sequence"] == 999
    assert flow.snapshot()["last_receipt_timestamp"] == 999
    ...
    assert flow.poll() == ()
    assert flow.snapshot()["receipt_pending"] == 0
    assert flow.admit([receipt(1000)]) == ()
    fresh = RfuCadence(lambda: clock[0])
    assert fresh.snapshot()["receipt_sequence"] == 0
    assert fresh.admit([receipt(1001)]) == (receipt(1001),)
...
def test_receipt_disconnect_is_final_and_wrong_flags_still_fail():
    flow = RfuCadence(lambda: 0)
...
async def deadline_path(paced, *, ack_period=24, cadence=RfuCadence):
    """60Hz native repeats, ~15fps local service, real converter/FIFO/Reliable.

    Direct parent input bypasses native ingress Reliable backpressure. This
    models an unthrottled *service deficit*, not exact over-air packet loss.
    ...
    """
    clock = [0.0]
    flow = cadence(lambda: clock[0]) if paced else None
    translator = TranslatorFixture.connected()
...
def test_legacy_receipt_loss_masks_unthrottled_service_deficit():
    old, old_backlog = asyncio.run(deadline_path(False))
    new, new_backlog = asyncio.run(deadline_path(True, cadence=LegacyReceiptCadence))
